        ...
        <div class="col-md-12">
            <div class="panel panel-inverse">
                ...
                <div class="panel-body auto-height hidedesk">
                    <table id="otherMaterielTable"></table>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    name: 'NCAOtherInMaterielControl',
    methods: {
        queryParams: function (params) {
            params.otherstock_id = this.otherstock_id;
...
//        },
        search: async function() {
            $('#otherMaterielTable').bootstrapTable("refresh");
        },
        initTable: function () {
            this.masterTable.bootstrapTable({
                method: 'POST',
                url: apiUrl + 'getOtherListForMateriel',
...
//                    this.selectFormat('warehouse_zone', '仓区', this.zoneArray),
                    ...
                    common.BTRowFormatAlignRight('apply_amount', '需求数量'),
                ...
                onEditableSave: (field, row, oldValue, $el) => {
                    common.rowModifyWithT(this, apiUrl + 'modify', row, 'otherstock_id', this.masterTable)
                }
            });
...
//                this.zoneArray = this.pagePara.wareHouseZoneInfo;
        ...
        addO: function () {
            let params={
                otherstock_id: this.otherstock_id,
                stockapply_id: this.stockapply_id,
                other_bill_code: String(new Date().getTime()),
                warehouse_id: this.warehouse_id,
                materiels:this.materiels
            }

            this.$http.post(apiUrl + 'add', params).then((response) => {
...
//                this.materiels = []
    ...
    mounted: function() {
        ...
        this.otherstock_id = queryData.otherstock_id;
        this.stockapply_id = queryData.stockapply_id;
        this.warehouse_id = queryData.warehouse_id;
        this.warehouse_zone_id = queryData.warehouse_zone_id;
        this.masterTable = $('#otherMaterielTable');
        this.initPage();
    }
}
</script>
