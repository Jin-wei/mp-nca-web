                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-info-reject" v-on:click="acceptanceCheck('1', $event)">不同意</button>
                            <button type="button" class="btn btn-info" v-on:click="acceptanceCheck('2', $event)">同意</button>
                        </div>
                    </div>
                </div>
            </div>
      ...
      order: function() {
        ...
        function operateFormatter(value, row, index) {
          if (row.acceptance_state === 'CHECKING' && _self.userinfo.user_id === _self.order.order_supervision){
            return [
              '<a class="btn btn-info btn-xs m-r-5 detail" style="cursor: pointer">',
              '查看</i>',
              '</a>',
              '<a class="btn btn-info btn-xs m-r-5 check" style="cursor: pointer">',
              '验收</i>',
              '</a>'
            ].join('')
          } else {
        ...
        function stateTypeFilter(value, row) {
        ...
        function initTable() {
          $table.bootstrapTable({
            ...
            columns: [
              ...
              common.BTRowFormatAlignLeft('acceptance_submit_name', '提交人'),
